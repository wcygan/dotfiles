...
Enables Codex to intelligently configure Zed editor settings, keybindings, language servers, formatters, themes, and AI agents through direct modification of `config/zed/settings.json` and `config/zed/keymap.json`.
...
- External agents (Codex, Gemini CLI, custom)
...
```bash
.codex/skills/zed-config/scripts/validate-config.sh
```
