...
description: Five ready-to-use SKILL.md templates for code analysis, test generation, documentation, data processing, and forked analysis — adapted for global skills
tags: [templates, code-analysis, test-generation, documentation, data-processing, forked-analysis]
---

# Skill Templates

Five templates to use as starting points. These are global skills — avoid hard-coding
project-specific tech stack assumptions. Make any framework/language mentions conditional.
...
Write tests matching the project's existing test framework.
Read existing test files to match the pattern before generating new ones.

Common patterns:
- **Deno**: `@std/assert` + `@std/testing/bdd`
- **Node/Jest**: `describe`/`it`/`expect`
- **Go**: `testing.T` with table-driven tests
- **Rust**: `#[cfg(test)]` + `#[test]`
...
- Adjacent to source: `component.test.ts` / `_test.go` / `_test.rs`
- Or in `/tests/` directory matching source structure
...
---
name: doc-generator
description: Generate comprehensive documentation including READMEs, API docs, inline comments, and architecture guides. Use when documenting code, creating READMEs, or writing API documentation. Keywords: documentation, docs, README, API docs, comments, guide, architecture
---
