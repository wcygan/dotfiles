...
- Spawn the `luna_worker` custom agent through the native subagent tool by passing `agent_type: "luna_worker"` and omitting model and reasoning overrides so its tracked TOML owns the profile and bounded instructions. Record the returned agent ID and use native wait, steer, and close operations for that same worker.
- If a separate visible task is required and its launcher cannot select a custom agent, create the equivalent profile with `model: "gpt-5.6-luna"` and `thinking: "max"`, retain the bounded delegated-work contract in the prompt, and number Luna tasks as `[Worker N]`.
- Use a `gpt-5.6-sol` Smart Worker with `high` reasoning only when the user explicitly requests it or concrete task evidence shows Luna is unsuitable; explain the deviation before creation and prefix a visible Smart Worker task with exactly `[smart-worker]`. Preserve each worker's selected profile on follow-up turns by omitting model and reasoning overrides. If `luna_worker` is not discovered, report the mismatch and try a fresh task or Codex restart before substituting an equivalent profile.
...
- Keep the selected profile during recoverable errors. Improve missing criteria, dependencies, tool routing, or validation before considering a different worker, and do not promote an existing Luna worker to Smart Worker mid-task.
