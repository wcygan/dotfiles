...
- Run the supervisor on `gpt-5.6-sol` with `high` reasoning. Select that profile before invoking the workflow because the current task cannot retune itself. Verify the current setting when it is exposed; otherwise disclose that the profile is assumed.
- Create the worker with `model: "gpt-5.6-terra"` and `thinking: "medium"`. Keep that profile on follow-up turns by omitting model overrides. If the selected host does not support it, report the mismatch instead of silently substituting another model or reasoning level.
...
- Keep the configured model split during recoverable errors. Improve missing criteria, dependencies, tool routing, or validation before considering more reasoning, and change the profile only when the user explicitly requests it.
