...
| `21-animated-graph-data-flow.html` | "animated graph", "data flow", "show data moving", "fanout", "gossip", "flooding", "routing topology", "dependency graph", "packet propagation", "state moves through nodes" | User needs a step-by-step workflow with commands, owners, or failure branches more than topology | `13` for clickable workflow detail, `15` for adjustable concept parameters, `07` for pure timing/easing | SVG node-edge graph, labeled active edges, muted inactive topology, state chips, duplicate/visited indicators, reduced-motion fallback | CSS animation, optional replay/toggles | Browser-check active edges, label fit, narrow viewport, reduced-motion readability, and that animation encodes direction or state rather than decoration. |
...
- Choose `21` over `13` when a node-edge topology and moving data explain the idea; choose `13` when the reader mainly needs ordered process steps, commands, owners, or failure paths.
- Choose `21` over `07` when motion represents domain behavior in a graph; choose `07` when the only question is UI timing, easing, or component feedback.
