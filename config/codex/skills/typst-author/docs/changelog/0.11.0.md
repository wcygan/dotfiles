...
- Tables are now *much* more flexible, read the new Table Guide to get
  started
...
*Thanks to <https://github.com/PgBiel> for his work on tables!*
...
- Added *context expressions:* Read the chapter on context to get
  started
...
- The following existing functions have been made contextual: `query`,
  `locate`, `measure`, `counter.display`, `counter.at`, `counter.final`,
  `state.at`, and `state.final`
...
- The `locate` function now returns the location of a selector's unique
  match. Its old behavior has been replaced by context expressions and
  only remains temporarily available for compatibility.

- The `counter.at` and `state.at` methods are now more flexible: They
  directly accept any kind of locatable selector with a unique match
  (e.g. a label) instead of just locations
...
- The `state.display` function should not be used anymore, use
  `state.get` instead (Deprecation planned)
...
- Show-set rules on the same element (e.g.
  `{show heading.where(level: 1): set heading(numbering: "1.")}`) now
  work properly

- Setting properties on an element within a transformational show rule
  (e.g. `{show heading: it => { set heading(..); it }}`) is **not**
  supported anymore (previously it also only worked sometimes); use
  show-set rules instead **(Breaking change)**
...
- Added `reflow` argument to `rotate` and `scale` which lets them affect
  the layout
...
- Added `depth` and `offset` arguments to heading to increase or
  decrease the heading level for a bunch of content; the heading syntax
  now sets `depth` rather than `level` **(Breaking change)**

- List markers now cycle by default

- The `quote` function now more robustly selects the correct quotes
  based on language and nesting
...
- Added support for adjusting the `size` of accents relative to their
  base
...
- Added `row-type` argument to `csv` function to configure how rows will
  be represented
...
- The token `context` is now a keyword and cannot be used as an
  identifier anymore **(Breaking change)**
