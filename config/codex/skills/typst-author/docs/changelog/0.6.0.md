...
- Added argument to enable `limits` conditionally depending on whether
  the equation is set in `display` or `inline` style
...
- Added support for alternating page margins with the `inside` and
  `outside` keys

- Added support for specifying the page `binding`

- Added `to` argument to pagebreak function to skip to the next even or
  odd page
