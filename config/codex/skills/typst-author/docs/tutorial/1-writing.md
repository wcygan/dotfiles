...
Typst supports images in the formats PNG, JPEG, GIF, SVG, PDF, and WebP.
To add an image file to your project, first open the *file panel* by
...
string of text, so we need to enclose it in double quotes.
...
The `width` argument is a relative length. In our case, we specified a
percentage, determining that the image shall take up `{70%}` of the
page's width. We also could have specified an absolute value like
`{1cm}` or `{0.7in}`.
...
function. This function takes the figure's contents as a positional
argument and an optional caption as a named argument.
...
by writing an `[@]` symbol followed by that name. Headings and equations
can also be labelled to make them referenceable.
...
You can add a bibliography to your document with the `bibliography`
...
bibliography styles. Consult the reference for more details.
...
insert a column vector, we can use the `vec` function. Within math mode,
...
`cal` function is used to typeset calligraphic letters commonly used for
sets. The math section of the reference provides a complete list of all
...
be preceded with `#sym.` there. See the symbols section for a list of
...
insert into your document, such as tables, shapes, and code blocks. You
