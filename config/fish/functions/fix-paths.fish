function fix-paths --description 'Reset and rebuild PATH with correct priority'
    ...
    # 1. System essentials as fallback paths.
    set -l new_path /usr/local/bin /usr/bin /bin /usr/sbin /sbin

    # 2. User manual installs.
    ...
    # 4. Nix profile bins first for reproducible tools.
    test -d /nix/var/nix/profiles/default/bin; and set new_path /nix/var/nix/profiles/default/bin $new_path
    test -d $HOME/.nix-profile/bin; and set new_path $HOME/.nix-profile/bin $new_path
