...
async function runCommand(cmd: string[], shell: string): Promise<{success: boolean, output: string}> {
  try {
...
async function testShellFeature(description: string, command: string[], shell: "bash" | "zsh"): Promise<boolean> {
...
async function main(): Promise<void> {
  ...
  const platformExists = await checkFileExists(".platform");
  const aliasesExists = await checkFileExists(".aliases");
  const functionsExists = await checkFileExists(".functions");
  ...
  if (bashScore === bashTests.length) {
    ...
    console.log(`  ${colors.yellow}current_shell${colors.reset}     # See which shell you're using`);
    ...
    console.log(`  ${colors.yellow}hg pattern${colors.reset}       # Enhanced history search (bash only)`);
  } else {
    printFailure(`Only ${bashScore}/${bashTests.length} bash features working. Check configuration.`);
    Deno.exit(1);
  }
}
